...
def snapshot():
    ...
    with open("datasets/symbols.csv") as f:
        ...
        for symbol in symbols:
            ...
            if not df.empty:
                ...
                df["fastk"], df["fastd"] = talib.STOCHRSI(
                    df["Close"], int(stoch_length), int(stoch_k), int(stoch_d), 0
                )
                df["StochRSIflag"] = "neutral"
                ##Stoch conditions to test for
                ### k val < 20 and crossing above d val => Buy pressure
                StochBuyCondition = (
                    (df["fastk"] < float(stoch_buy_level))
                    & (df["fastk"].shift(1) < df["fastd"].shift(1))
                    & (df["fastk"] >= df["fastd"])
                )
                df["StochRSIflag"] = np.where(
                    StochBuyCondition, "Buy Pressure", df["StochRSIflag"]
                )

                ### k val > 80 and crossing above d val => Buy pressure
                StochSellCondition = (
                    (df["fastk"] > float(stoch_sell_level))
                    & (df["fastk"].shift(1) > df["fastd"].shift(1))
                    & (df["fastk"] <= df["fastd"])
                )
                df["StochRSIflag"] = np.where(
                    StochSellCondition, "Sell Pressure", df["StochRSIflag"]
                )

                # macd, macdsignal, macdhist = MACD(close, fastperiod=12, slowperiod=26, signalperiod=9)
                df["macd"], df["macdsignal"], df["macdhist"] = talib.MACD(
                    df["Close"], int(macd_fp), int(macd_slp), int(macd_sp)
                )
                df["MACDflag"] = "neutral"
                ...
                df["EMA_fast"] = EMAIndicator(
                    df["Close"], int(ema_fast)
                ).ema_indicator()
                df["EMA_medium"] = EMAIndicator(
                    df["Close"], int(ema_medium)
                ).ema_indicator()
                df["EMA_slow"] = EMAIndicator(
                    df["Close"], int(ema_slow)
                ).ema_indicator()
                ...
                df["MACDflag"] = np.where(
                    MacdBuyCondition, "Buy Signal", df["MACDflag"]
                )
                ...
                df["MACDflag"] = np.where(
                    MacdBuyCondition, "Sell Signal", df["MACDflag"]
                )
                ...
                CipherBullCond = (df["wt1"].shift(1) < df["wt2"].shift(1)) & (
                    df["wt1"] >= df["wt2"]
                )
                df["CipherFlag"] = np.where(
                    CipherBullCond, "Buy Signal", df["CipherFlag"]
                )

                CipherSellCond = (df["wt1"].shift(1) > df["wt2"].shift(1)) & (
                    df["wt1"] <= df["wt2"]
                )
                df["CipherFlag"] = np.where(
                    CipherSellCond, "Sell Signal", df["CipherFlag"]
                )
    ...
    MACDVals = {"MACD Slow": macd_slp, "MACD Fast": macd_fp, "MACD Signal": macd_sp}
    EMAVals = {"EMA Fast": ema_fast, "EMA Medium": ema_medium, "EMA Slow": ema_slow}
    CIPHERVals = {
        "n1": cip_n1,
        "n2": cip_n2,
        "obl1": cip_obl1,
